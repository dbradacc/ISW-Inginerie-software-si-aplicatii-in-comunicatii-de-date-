...

{
    ...
    /* statistici pe semestrul 1 și 2 cu filtru de timp */
    public function stats(string $range = 'all'): array
    {
        $where = "";
        
        // Logica de filtrare a timpului
        if ($range === '24h') {
            $where = " AND p.Data >= DATE_SUB(NOW(), INTERVAL 1 DAY)";
        } elseif ($range === '7d') {
            $where = " AND p.Data >= DATE_SUB(NOW(), INTERVAL 7 DAY)";
        } elseif ($range === '30d') {
            $where = " AND p.Data >= DATE_SUB(NOW(), INTERVAL 30 DAY)";
        }

        $sql = "
          SELECT s.ID_Student,
                 CONCAT(s.Nume,' ',s.Prenume) AS Student,
                 SUM(CASE WHEN p.Semester = 1 AND p.Status = 'prezent' THEN 1 ELSE 0 END) AS Sem1,
                 SUM(CASE WHEN p.Semester = 2 AND p.Status = 'prezent' THEN 1 ELSE 0 END) AS Sem2
          FROM Prezente p
          JOIN Studenti s ON s.ID_Student = p.ID_Student
          WHERE 1=1 $where
